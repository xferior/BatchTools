...
def distribute_hosts(total_batches, total_hosts):
    ...
    for i in range(total_batches):
        if distribution[i] > 1:
            ...
            # Adjust another batch to balance the change
            # and maintain total hosts
            adjust_index = (
                (i + random.randint(1, total_batches - 1)) % total_batches
            )
...
if __name__ == "__main__":
    ...
    # Check for the correct number of arguments, ensure they are only numbers
    if len(sys.argv) != 4 or not all(is_number(arg) for arg in sys.argv[1:]):
        print(
            "Usage: python script.py"
            " <total_batches> <total_hosts> <output_file_number>"
        )
        sys.exit(1)
