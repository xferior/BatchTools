...
def read_batches(file_path):
    ...
    # Return a list of tuples (BatchID, NumberOfHosts),
    # excluding batches with 0 hosts
    with open(file_path, 'r') as file:
        return [
            (line.split(',')[0], int(line.split(',')[1]))
            for line in file if int(line.split(',')[1]) > 0
        ]
...
def distribute_batches(batches, enabled_nodes):
    ...
    for batch, hosts in sorted(batches, key=lambda x: x[1], reverse=True):
        ...
        least_loaded_node = min(
            nodes,
            key=lambda x: (nodes[x]['total_hosts'], len(nodes[x]['batches']))
        )
...
def print_usage():
    ...
    print("<CT>: Ticket ID")
    print(
        "--no-script: Optional argument"
        " to print NODE information without generating script.\n"
    )
